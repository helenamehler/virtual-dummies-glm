...
// experiments/sec4_2_memory_runtime/ad_afs_logistic.hpp
//
// AD_AFS_Logistic -- Augmented-Dummy Adaptive Forward Stepwise with a logistic
// link. C++ port of experiments/sec4_1_equivalence/AD_AFS_Logistic.py, written as the explicit-
// augmentation counterpart of src/vd_afs_logistic.cpp for the memory /
// runtime benchmark.
//
// Fabian's TRexSelector C++ library covers the linear model only
// (trex::tsolvers::linear_model::...), so there is no upstream logistic
// baseline to benchmark against. This is it.
//
// Same deliberate difference from the Python reference as ad_afs_poisson.hpp:
// X and D are kept as SEPARATE column-major maps instead of a concatenated
// [X | D]. Concatenating would allocate a second n*(p+L) block and double the
// peak RSS, which would make the benchmark measure the copy rather than the
// algorithm. Candidate indices run 0..p-1 over X and p..p+L-1 over D, so
// selection, the active set and the IRLS refit behave exactly as on the
// augmented matrix.
//
// Everything else mirrors the Python line for line:
//   - labels y_binary = 1[y > 0], recovered from the centered response exactly
//     as VD_AFS_Logistic::init_logistic_ does
//   - null intercept b0 = logit(pbar), pbar = clamp(mean(y_binary), 1e-6, 1-1e-6)
//   - prob = sigmoid(clamp(eta, +-30)), score s = y_binary - prob, then CENTERED
//   - selection on |corr| with corr = [X D]^T s
//   - IRLS on [1 | active columns]: p_a clamped to [1e-10, 1-1e-10],
//     w = p_a (1 - p_a), z = eta + (y - p_a)/w, normal equations on the
//     sqrt(w)-scaled design with an eps ridge on the diagonal, warm-started
...
//     same object VD_AFS_Logistic::run() returns, so both sides pay for it.
#pragma once
...
public:
  ...
  struct Options {
    ...
    double eps           = 1e-12;
    int    max_steps     = -1;    // <0 -> p + T, matching VD_AFS_Logistic::run
  };

  // X: n x p column-major, D: n x L column-major, y: centered response; the
  // binary labels are recovered as 1[y > 0].
  // Columns of X and D are assumed centered and unit-L2 (the harness does it).
  AD_AFS_Logistic(const double* Xptr, int n, int p,
...
private:
  static constexpr double kEtaClamp = 30.0;   // matches VD_AFS_Logistic
  static constexpr double kPLo      = 1e-10;  // probability clamp in the IRLS
  static constexpr double kPHi      = 1.0 - 1e-10;
  static constexpr double kWFloor   = 1e-12;  // floor on the IRLS weights
  ...
  Vec y_binary_, eta_, prob_, score_, beta_, corr_, nu_;
  ...
  MatC Xw_, G_;
  Vec  theta_, theta_new_, eta_a_, p_a_, w_, sw_, z_, zw_, rhs_, ba_;

  static Vec mean_fn_(const Vec& e) {
    return (1.0 + (-e.array().min(kEtaClamp).max(-kEtaClamp)).exp()).inverse().matrix();
  }

  void init_() {
    y_binary_.resize(n_);
    for (int i = 0; i < n_; ++i) y_binary_(i) = (y_(i) > 0.0) ? 1.0 : 0.0;

    const double pbar = std::min(std::max(y_binary_.mean(), 1e-6), 1.0 - 1e-6);
  ...
  void irls_solve_() {
    ...
    for (int iter = 0; iter < opt_.irls_max_iter; ++iter) {
      ...
      p_a_ = mean_fn_(eta_a_).cwiseMax(kPLo).cwiseMin(kPHi);
      w_   = (p_a_.array() * (1.0 - p_a_.array())).matrix().cwiseMax(kWFloor);
      z_   = eta_a_ + (y_binary_ - p_a_).cwiseQuotient(w_);
      sw_  = w_.cwiseSqrt();
