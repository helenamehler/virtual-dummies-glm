"""
Sec. 4.1, Fig. 1, Cox column. FDP and TPP of AD-AFS-Cox, VD-AFS-Cox,
VD-AFS-Gauss on the observed times (misspecified, event indicator discarded)
and model-X knockoffs against the SNR.

Censored times have no conditional variance function, so the SNR is taken on
the latent log-time scale: with an exponential baseline hazard,
log T = -eta + xi with Gumbel xi of variance pi^2/6, and SNR = Var(eta)/(pi^2/6),
see snr_glm.py. The linear predictor is scaled in closed form, so ``snr_exact``
equals ``snr``. The censoring rate is held at ``cens``; ``cens_realized`` logs
the realised rate. The knockoff arm needs knockpy, skglm and scikit-survival and
...
# skglm and Coxnet are compiled; one worker per core means one thread each.
...
from AD_AFS_Cox import AD_AFS_Cox

# Unified SNR axis (see snr_glm.py). Cox form: SNR = (1-c) Var(eta),
# i.e. the event rate times the spread of the risk scores.
from snr_glm import snr_cox, unit_variance, scale_cox

# CoxKnockoff arm (Li, Yu and Zhao 2023). Optional on purpose: the cluster
# environment may not carry knockpy and scikit-survival, and a missing
# competitor must not take the sweep down with it.
try:
...
# still only grants ``-c`` * ``--mem-per-cpu``. The AD baseline holds K
# persistent [X|D] copies (~n*p*(1+L)*8 bytes each) per worker, so an
...
def slurm_n_jobs(default=-1, cap=None):
    ...
    workers means MORE memory per worker.
    """
    n = default
...
# ================================================================
# Problem generation (Cox model, SNR sweep)
# ================================================================
...
def _solve_cens_scale(Ttrue, target, lo=1e-8, hi=None, tol=1e-10, max_iter=200):
    """Exponential censoring scale c with E[censoring rate] == target.

    For C ~ Exp(scale = c) independent of Ttrue,
        P(C < T_i) = 1 - exp(-T_i / c),
    so the expected censoring rate is mean_i(1 - exp(-T_i/c)), which is strictly
    decreasing in c -> bisection. This replaces the quantile heuristic, which
    only hits the target rate approximately and — crucially — MISSES IT BY A
    DIFFERENT AMOUNT AT EACH AMPLITUDE (exp(eta) rescales the time distribution),
    silently confounding the sweep with a varying censoring rate.
    """
    T = np.asarray(Ttrue, float)
    if hi is None:
        hi = max(float(T.max()) * 1e4, 1.0)
    for _ in range(max_iter):
        mid = 0.5 * (lo + hi)
        rate = float(np.mean(1.0 - np.exp(-T / mid)))
        if rate > target:      # too much censoring -> larger scale
            lo = mid
        else:
            hi = mid
...
def sample_y_snr(X, beta_base, snr, cens, rng, cens_calibrate=True):
    """Survival times / censoring at target SNR ``snr``, censoring rate fixed.

    SNR = Var(eta)/(pi^2/6) on the latent log-time scale, so
    eta = sqrt(SNR * pi^2/6) * eta_unit hits the target exactly and the
    censoring rate does not enter it. Returns
    (time, delta, snr_exact, cens_realized); ``snr_exact`` is the target up to
    floating point, ``cens_realized`` is the quantity that actually varies.
    """
    eta = scale_cox(float(snr)) * unit_variance(X @ beta_base)
    Ttrue = -np.log(rng.uniform(size=X.shape[0])) / np.exp(eta)    # inverse-CDF exp
    if cens <= 0.0:
        time = Ttrue
        delta = np.ones(X.shape[0])
    else:
        scale = (_solve_cens_scale(Ttrue, float(cens)) if cens_calibrate
                 else float(np.quantile(Ttrue, 1.0 - cens)))
        C = rng.exponential(scale=scale, size=X.shape[0])
        time = np.minimum(Ttrue, C)
        delta = (Ttrue <= C).astype(float)
    return (time.astype(float), delta, snr_cox(eta),
            float(1.0 - delta.mean()))
...
# ================================================================
# AD-AFS-Cox-T-Rex (Python AD_AFS_Cox)
# ================================================================
def trex_ad_select(X, time, delta, *, tFDR, K, num_dummies, Tmax, rho, seed):
    ...
    for _ in range(K):
        ...
        tl = AD_AFS_Cox(XD, np.asarray(time), np.asarray(delta), int(num_dummies),
                        rho=rho, normalize=False, eps=EPS, verbose=False)
        solvers.append(tl)
...
# ================================================================
# Shared TRexOptions builder for the VD arms
# ================================================================
def _base_opts(*, tFDR, K, L_factor, Tmax, seed):
    opt = TRexOptions()
    opt.tFDR = float(tFDR)
    opt.K = int(K)
...
# ================================================================
# VD-AFS-Cox-T-Rex (C++ vd_selectors)
# ================================================================
def trex_vd_select(X, time, delta, *, tFDR, K, L_factor, Tmax, rho, seed, efron=False):
    opt = _base_opts(tFDR=tFDR, K=K, L_factor=L_factor, Tmax=Tmax, seed=seed)
    opt.solver = SolverType.AFS_Cox  # type: ignore
    opt.rho = float(rho)
    opt.cox_status = np.ascontiguousarray(delta, dtype=float)  # type: ignore
    opt.cox_efron = bool(efron)  # type: ignore

    sel = TRexSelector(opt)
    # Cox solver consumes the raw event/censoring TIMES; delta via cox_status.
    res = sel.run(np.asfortranarray(X), np.ascontiguousarray(time, dtype=float))
    return (np.asarray(res.selected_var, dtype=int), int(res.T_stop),
            float(res.v_thresh), len(res.selected_var))


# ================================================================
# VD-AFS-Gauss-T-Rex — misspecified Gaussian/OLS AFS on centered log(time)
#
# Same solver family and the same ``rho`` as VD-AFS-Cox, only the likelihood
# differs, so the pair isolates the value of the Cox partial likelihood (and of
# using delta at all — it is discarded here).
# ================================================================
def trex_vd_afs_gauss_select(X, time, *, tFDR, K, L_factor, Tmax, rho, seed):
    opt = _base_opts(tFDR=tFDR, K=K, L_factor=L_factor, Tmax=Tmax, seed=seed)
    opt.solver = SolverType.AFS  # type: ignore
    opt.rho = float(rho)

    sel = TRexSelector(opt)
    y_c = np.log(np.asarray(time, dtype=float))
    y_c = y_c - y_c.mean()
...
# ================================================================
def knockoff_select(X, time_, delta, *, alpha, seed):
    y = np.column_stack([np.asarray(time_, dtype=float),
                         np.asarray(delta, dtype=float)])
    out = kog.glm_knockoff(np.ascontiguousarray(X, dtype=float), y, "cox",
...
# ================================================================
def one_rep(rep, snr, alpha, K, L, Tmax, rho, seed0, *, n, p, s, cens):
    ...
    time, delta, snr_exact, cens_realized = sample_y_snr(
        X, beta_base, float(snr), float(cens), rng)

    sel_ad, *_ = trex_ad_select(
        X, time, delta, tFDR=alpha, K=K, num_dummies=X.shape[1]*L,
        Tmax=Tmax, rho=rho, seed=int(rng.integers(2**31)),
    )
    sel_vd, *_ = trex_vd_select(
        X, time, delta, tFDR=alpha, K=K, L_factor=L, Tmax=Tmax, rho=rho,
        seed=int(rng.integers(2**31)),
    )
    sel_gs, *_ = trex_vd_afs_gauss_select(
        X, time, tFDR=alpha, K=K, L_factor=L, Tmax=Tmax, rho=rho,
        seed=int(rng.integers(2**31)),
    )
    # A removed arm drew one seed here; keep the draw so the knockoff arm
    # sees the seed stream that produced the results.
    _ = rng.integers(2**31)
    ...
    if KNOCKOFF_OK:
        try:
            sel_ko = knockoff_select(
                X, time, delta, alpha=alpha, seed=int(rng.integers(2**31)))
    ...
    out.update({"L": int(L), "snr": float(snr), "rep": int(rep),
                "snr_exact": snr_exact, "cens_realized": cens_realized})
    return out
...
# ================================================================
def sweep(snrs, Ls, *, cens=0.4, alpha=0.1, reps=50, K=20, Tmax=20, rho=1.0,
            ...
            for snr in snrs:
                res = Parallel(n_jobs=n_jobs, backend="loky")(
                    delayed(one_rep)(
                        ...
                        float(rho), int(seed0), n=n, p=p, s=s, cens=float(cens),
                    ) for r in range(reps)
                )
                res = [r for r in res if r is not None]
                if not res:
                    continue
                extra = {c: float(np.mean([d[c] for d in res])) for c in ("snr_exact", "cens_realized")}
                for key, name in METHODS.items():
                    ...
                    r_ = np.sqrt(len(vals))
                    rows.append({
                        "L": L, "snr": snr, "cens": cens, **extra, "method": name,
...
# ================================================================
# Main: writes results/sec4_1_equivalence/cox/summary.csv and meta.json
# ================================================================
if __name__ == "__main__":
    outdir = Path("results") / "sec4_1_equivalence" / "cox"
    outdir.mkdir(parents=True, exist_ok=True)

    snrs = (0.1, 0.25, 0.5, 1.0, 2.0, 4.0)
    cens = 0.4          # right-censoring rate, fixed across the sweep
    Ls = (5,)
    alpha = 0.1
    reps = 500
    K = 20
    ...
    seed0 = 12
    n, p, s = 300, 1000, 10
    max_workers = 10      # the AD arm holds K augmented designs per worker

    df = sweep(snrs=snrs, Ls=Ls, cens=cens, alpha=alpha, reps=reps, K=K,
               Tmax=Tmax, rho=rho, seed0=seed0, n_jobs=slurm_n_jobs(cap=max_workers),
               n=n, p=p, s=s)
    df.to_csv(outdir / "summary.csv", index=False)
    meta = dict(snrs=list(snrs), cens=cens, Ls=list(Ls), alpha=alpha, reps=reps,
