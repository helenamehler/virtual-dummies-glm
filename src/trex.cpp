...
VDOptions TRexSelector::make_vd_opts_(unsigned long long seed,
  ...
  o.cox_status      = opt_.cox_status;   // separate censoring indicator (Cox)
  o.cox_efron       = opt_.cox_efron;
  return o;
}
    ...
    int num_dummies, unsigned long long seed, int T_stop) const
{
  ...
  switch (opt_.solver) {
    ...
    case SolverType::AFS_Poisson:
      return std::make_unique<VD_AFS_Poisson>(X, y, num_dummies, vd);
    case SolverType::AFS_Multinomial:
      return std::make_unique<VD_AFS_Multinomial>(X, y, num_dummies, vd);
    case SolverType::AFS_Cox:
      return std::make_unique<VD_AFS_Cox>(X, y, num_dummies, vd);
  }
  return std::make_unique<VD_LARS>(X, y, num_dummies, vd);
}
    ...
    const Eigen::Ref<const Vec>& y)
{
  ...
  if (opt_.verbose) {
    const char* solver_names[] = {"LARS", "OMP", "AFS", "AFS_Logistic", "AFS_Poisson", "AFS_Multinomial", "AFS_Cox"};
