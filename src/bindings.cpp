...
PYBIND11_MODULE(vd_selectors, m) {
  ...
  py::class_<VDOptions>(m, "VDOptions")
    ...
    .def_readwrite("cox_status",      &VDOptions::cox_status)
    .def_readwrite("cox_efron",       &VDOptions::cox_efron);
  ...
  // VD_AFS_Poisson
  // ==================================================================
  auto afs_pois = py::class_<VD_AFS_Poisson>(m, "VD_AFS_Poisson")
    .def(py::init(&make_solver<VD_AFS_Poisson>),
        py::arg("X"), py::arg("y"), py::arg("num_dummies"), py::arg("options"),
        py::keep_alive<1,2>(), py::keep_alive<1,3>())
    .def("run", [](VD_AFS_Poisson& self, int T) {
          py::gil_scoped_release release;
          return self.run(T);
        }, py::arg("T") = 1);
  bind_common<VD_AFS_Poisson>(afs_pois);

  // ==================================================================
  // VD_AFS_Multinomial
  // ==================================================================
  auto afs_multi = py::class_<VD_AFS_Multinomial>(m, "VD_AFS_Multinomial")
    .def(py::init(&make_solver<VD_AFS_Multinomial>),
        py::arg("X"), py::arg("y"), py::arg("num_dummies"), py::arg("options"),
        py::keep_alive<1,2>(), py::keep_alive<1,3>())
    .def("run", [](VD_AFS_Multinomial& self, int T) {
          py::gil_scoped_release release;
          return self.run(T);
        }, py::arg("T") = 1);
  bind_common<VD_AFS_Multinomial>(afs_multi);

  // ==================================================================
  // VD_AFS_Cox
  // ==================================================================
  auto afs_cox = py::class_<VD_AFS_Cox>(m, "VD_AFS_Cox")
    .def(py::init(&make_solver<VD_AFS_Cox>),
        py::arg("X"), py::arg("y"), py::arg("num_dummies"), py::arg("options"),
        py::keep_alive<1,2>(), py::keep_alive<1,3>())
    .def("run", [](VD_AFS_Cox& self, int T) {
          py::gil_scoped_release release;
          return self.run(T);
        }, py::arg("T") = 1);
  bind_common<VD_AFS_Cox>(afs_cox);
  ...
  py::enum_<SolverType>(m, "SolverType")
    ...
    .value("AFS_Poisson", SolverType::AFS_Poisson)
    .value("AFS_Multinomial", SolverType::AFS_Multinomial)
    .value("AFS_Cox", SolverType::AFS_Cox)
    .export_values();
  ...
  py::class_<TRexOptions>(m, "TRexOptions")
    ...
    .def_readwrite("cox_status",     &TRexOptions::cox_status)
    .def_readwrite("cox_efron",      &TRexOptions::cox_efron);
