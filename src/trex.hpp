...
// ---------- Enums ----------
enum class SolverType : uint8_t { LARS = 0, OMP = 1, AFS = 2, AFS_Logistic = 3, AFS_Poisson = 4, AFS_Multinomial = 5, AFS_Cox = 6 };
...
struct TRexOptions {
  ...
  // ---- Cox proportional-hazards extras (only used when solver == AFS_Cox) ----
  // y carries the event/censoring times; cox_status is the separate length-n
  // censoring indicator (1 = event, 0 = censored). Forwarded to VDOptions.
  Vec  cox_status;
  bool cox_efron = false;          // false = Breslow ties, true = Efron
};
