...
void VD_Base::init_common_state_() {
  ...
  // Growth buffers must hold every column run() can append. AFS/Poisson is not
  // OLS: with the ridge-regularized IRLS and the damped Poisson fit the active
  // set (real features + realized dummies) is bounded only by the loop length
  // max_steps = p_ + T_stop, NOT by min(n,p). Sizing these to min_np underflows
  // and writes past the block -> heap corruption (munmap_chunk / SIGABRT).
  const int grow_cap = std::max(n_, p_) + opt_.T_stop + 2;
  ...
  // Basis
  basis_.resize(n_, grow_cap);
  basis_indices_.clear();
  basis_indices_.reserve(grow_cap);

  // Reusable buffers
  proj_coeffs_.resize(std::max(1, grow_cap));
  ortho_buffer_.resize(n_);

  // Active columns & Cholesky
  X_active_.resize(n_, grow_cap);
...
std::optional<Vec> VD_Base::orthonormalize_(const Vec& v) const {
  ...
  constexpr double tol = 1e-10;                 // relative Toleranz
  if (vnorm <= tol) return std::nullopt;        // v selbst ~ 0

  const int nb = basis_size_();
  if (nb == 0) return Vec(v / vnorm);
  ...
  double nu = ortho_buffer_.norm();

  // 2. Reorthogonalisierung, falls stark geschrumpft (DGKS-Schwelle 1/√2 ≈ 0.7)
  if (nu < 0.7 * vnorm) {
    Vec c2 = basis_.leftCols(nb).transpose() * ortho_buffer_;   // Größe nb
    ortho_buffer_.noalias() -= basis_.leftCols(nb) * c2;
    proj_coeffs_.head(nb)   += c2;               // proj_coeffs_ bleibt = Qᵀv (gesamt)
    nu = ortho_buffer_.norm();
  }

  // relativer Rangtest: Rest vernachlässigbar ggü. Original -> schon im Spann
  if (nu <= tol * vnorm) return std::nullopt;
...
void VD_Base::realize_dummy_(int vd_idx, bool append_to_basis) {
